...
# ------------------------
# 紧急通知
我们目前面临了数据库的完全丢失，目前还在恢复中……这非常难，虽然BiliHP上线仅仅几个月，但是数据量多到大家不敢想（不包含log），我们的礼物数据增量差不多每秒67-138条（仅仅礼物表）


所以这几个月数据库就几十G了，但是用上了内存盘，其实IOPS能达到数百万级，不过风险确实很大，我们准备了UPS，准备了非常多措施来防止断电，尴尬的是，漏水一直是我们不太关注的，在老的机箱位置中，受限于水管的长度，水冷是直接放在机箱上部的，这次漏的就是机箱上部的那个水冷管，所以机箱被浇了一个透心凉，而且漏水的位置刚好是机箱顶板USB3.0的位置，3.0正下方就是SSD，所以……就凉凉，好在今年的架构，大部分的控制数据都是在大家本地的，即使数据丢失，也可以快速重设，算是解决了大问题了


另外这个项目是一个示范项目，所以如果用户数多一点可以，少一点也没有问题的，主要是炫技，其他的再说


https://github.com/tobycroft/BiliHP-APP/issues/25
